...
type User struct {
	...
	Language       int                  `json:"-"`
	RoleIds        communal.StringArray `json:"roleIds"`
	Groups         communal.StringArray
	OrgId          int64 `json:",string"`
	Ext            communal.JsonMap
	Status         int
	Path           communal.Int64Array //机构路径
}
type Org struct {
	communal.DBase `xorm:"extends"`
...
type JnOrg struct {
	...
	Progress       []communal.JsonMap `json:"progress" form:"progress"`
}
...
type MiniUser struct {
	...
	RoleIds        communal.Int64Array `json:"roleIds"` // 角色列表
	Email          string              `json:"email" form:"email"`
	Mobile         string              `json:"mobile" form:"mobile"`
